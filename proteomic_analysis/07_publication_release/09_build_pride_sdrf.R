...
field_status <- rbind(
  ...
  fs("comment[modification parameters]", "required", "MISSING_REQUIRED_METADATA", NOT_AVAILABLE,
     "no search-parameter record (DIA-NN log, config or report) exists in the project tree",
     "acquisition facility: DIA-NN report.log.txt or the search configuration", "yes",
     NA_character_),
  fs("comment[precursor mass tolerance]", "optional", "MISSING_OPTIONAL_METADATA", NOT_AVAILABLE,
     ...
     "acquisition facility: DIA-NN report.log.txt", "no", NA_character_),
  ...
  fs("comment[proteomics data acquisition method]", "required", "MISSING_REQUIRED_METADATA",
     ...
     paste("The quantification output has the DIA-NN column schema, which is suggestive of",
           "data-independent acquisition, but DIA-NN does not by itself establish the",
           "acquisition mode used. Deliberately not guessed.")),
...
missing_md <- c(
  ...
  "",
  "A DIA-NN run log (`report.log.txt`) plus the instrument method for the 2024-12-17",
  "acquisition would supply, in one go: the search-software version, the digestion enzyme,",
  "the modification parameters, the mass tolerances, the acquisition method, and the",
  "instrument model. Those six fields are the bulk of what is blocking a complete SDRF.",
  "",
  ...
  md_row(c("acquisition method = DIA from the DIA-NN column schema",
           "DIA-NN output establishes the software, not the acquisition mode used on the instrument")),
...
readme_pride <- c(
  ...
  md_row(c("DIA-NN search output (`report.tsv` / `report.parquet` / `.speclib`)",
           "**SEARCH output**",
  ...
  md_row(c("2", "Search / quantification output", "DIA-NN report / library",
  ...
  "2. Obtain the DIA-NN run log and the instrument method for the 2024-12-17 acquisition;",
