...
lineage <- rbind(
  ...
  lin("A03_search_quantification", "search / quantification output",
      "DIA-NN report / spectral library",
      "A02_acquisition_raw", "peptide and protein identification and quantification",
      "DIA-NN", UNKNOWN, "acquisition run", NA_character_, "yes",
      paste("ABSENT from the project tree:", UNRESOLVED, "-- no report.tsv, report.parquet,",
            "report.log.txt, .speclib or search configuration was found. Software identity",
            "is evidenced by the DIA-NN pg_matrix column schema on A04 (Protein.Group,",
            "Protein.Names, Genes, First.Protein.Description) and by sample_annotation.xlsx",
            "carrying a sheet named `report.stats_`. No version string exists.")),
  lin("A04_protein_group_matrix_prefilter", "protein-group matrix",
      ...
      "DIA-NN", UNKNOWN, "acquisition run", hash_or(p_pg_raw), "yes",
  ...
  lin("A13_enrichment", "GSEA and ORA results",
      ...
      paste("log2FC-ranked GSEA in the same run is a SENSITIVITY analysis only.",
...
gap <- c(
  ...
  paste0("| Quantification software | DIA-NN | The annotation columns are the DIA-NN",
         " `pg_matrix` schema (`T: Protein.Group`, `T: Protein.Names`, `T: Genes`,",
         " `T: First.Protein.Description`), and `sample_annotation.xlsx` carries a sheet",
         " named `report.stats_` |"),
  ...
  "",
  paste0("1. **It is standardised per protein.** Every one of the 5,349 proteins has mean 0",
         " and standard deviation 1 across the 96 acquisitions. So the values are neither",
         " log2 abundance nor raw intensity, and the token `unnormalized` in the filename is",
         " not a description of them. Corroborated independently by the canonical",
         " differential tables: limma's `AveExpr` lies within +/-0.0011 of zero for every",
         " protein, which requires a per-protein centred input."),
  ...
  "",
  "**Consequence for reporting.** The effect size the canonical tables call `log2fc` (and",
  "ProTigy calls `logFC`) is a difference of group means on this standardised scale -- a",
  "standardised mean difference in units of per-protein standard deviation, not a log2",
  "ratio. The publication release publishes it as `effect_size_sd_units`. No value was",
  ...
  "| The DIA-NN run log (`report.log.txt`) | search software version, enzyme, modifications, tolerances, acquisition method |",
...
external <- rbind(
  ...
  sv("DIA-NN", "external application", UNKNOWN, "MISSING_RECOVERABLE",
     ...
     paste("Software identity evidenced by the pg_matrix column schema and a",
           "report.stats-derived annotation sheet. Recoverable from the acquisition",
           "facility (report.log.txt).")),
