...
"",
"**How that mapping was established.** None of the four originally submitted files exists",
"under those names anywhere in the project tree -- a recursive search of the analysis root",
"and of the enclosing project folder returns no `processed_protein_group_matrix_*`, no",
"`GSEA_ORA_all_results*` and no `README.txt`. The correspondence above is therefore:",
"",
"- for the two matrices, an identification **by shape**: `pg.matrix_raw.txt` at the project",
"  root has exactly 5,747 protein rows and 96 measurement columns (plus 7 annotation",
"  columns), and the processed matrix has exactly 5,349 x 96. Both match the dimensions",
"  reported for the submitted files, and all 5,349 analysed protein identifiers are a",
"  strict subset of the 5,747;",
"- for `GSEA_ORA_all_results.xlsx`, an identification **by analysis generation**, not by",
"  inspection of the workbook. It is superseded because any hemisphere-level,",
"  log2FC-ranked GSEA/ORA output is superseded by the animal-level, moderated-t-ranked",
"  canonical run -- which holds whatever that workbook's internal layout was.",
"",
"If the original submission package is still held elsewhere, a direct file-level comparison",
"would let the first two rows be upgraded from a shape match to a byte match. Nothing in",
"this package depends on that.",
"",
