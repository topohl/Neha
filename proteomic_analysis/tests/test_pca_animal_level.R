...
fixture_path <- tempfile("neha_animal_pca_", fileext = ".gct")
...
prepared_repeat <- prepare_neha_animal_pca(validated$expression_matrix, center = TRUE, scale. = TRUE)
...
expect_error(
  validate_neha_pca_animal_input(duplicate_fixture),
...
expect_error(
  validate_neha_pca_animal_input(hemisphere_fixture),
  "duplicated within sample_class",
...
expect_error(
  validate_neha_pca_animal_input(misaligned_fixture),
  "not aligned",
...
expect_error(
  validate_neha_pca_animal_input(underfilled_fixture),
...
expect_error(
  validate_neha_pca_animal_input(nonfinite_fixture),
  "nonfinite values",
...
expect_error(
  validate_neha_pca_output_root(file.path(historical_root, "child"), historical_root),
  "cannot overwrite the historical PCA root",
  "Animal-level PCA must reject the historical output root and descendants."
)
isolated_root <- validate_neha_pca_output_root(file.path(tempdir(), "animal_pca"), historical_root)
expect_true(!neha_pca_path_is_within(isolated_root, historical_root),
            "An isolated animal-level PCA output root was rejected or rewritten incorrectly.")

audit <- make_neha_pca_audit(
...
expect_true(grepl("NEHA_PCA_ANIMAL_LEVEL_INPUT", pca_script, fixed = TRUE) &&
              grepl("NEHA_PCA_OUTPUT_ROOT", pca_script, fixed = TRUE),
