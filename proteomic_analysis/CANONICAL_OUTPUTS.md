...
| **Enrichment (GSEA/ORA)** | **`03_output/enrichment/enrichment_t_rank_validation_20260825/`** | `03_output/enrichment/enrichment_log2fc_rank_legacy/` | **Canonical GSEA ranks by the moderated `t` statistic.** The legacy folder ranked by `log2fc` (it was named just `enrichment/` before the restructure — renamed to make the difference explicit). log2fc-ranked results exist only as a *sensitivity* analysis. The statistic recorded as `log2fc` is a standardized abundance difference (SD units), not a log2 fold change — the analysis matrix is standardised per protein; see `07_publication_release/README.md`. Folder and column names keep the historical token so provenance holds. |
